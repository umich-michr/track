...
public class OriginValidatorImpl implements OriginValidator {
  ...
  public void validate(HttpServletRequest request) {
    ...
    final String resolvedOrigin = resolveOrigin(request);
  ...
  private String resolveOrigin(HttpServletRequest request) {
    logger.debug("Resolving origin from request");
  ...
  private String extractBaseUrl(String url) {
    try {
      java.net.URI uri = java.net.URI.create(url); // Use URI to parse the URL
      return uri.getScheme() + "://" + uri.getHost() +
          (uri.getPort() != -1 ? ":" + uri.getPort() : "");
    } catch (Exception e) {
      logger.debug("Failed to parse URL: {}", url, e);
      throw new ValidationException("Invalid URL format", HttpStatus.FORBIDDEN);
    }
  }
  ...
  private void validateResolvedOrigin(String resolvedOrigin, ClientConfiguration clientConfiguration) {
    logger.debug("Validating resolved origin: {}", resolvedOrigin);
    if (clientConfiguration.getAuthorizedOrigins().stream().noneMatch(resolvedOrigin::equalsIgnoreCase)) {
      logger.debug("Resolved origin doesn't correspond to the authorized origin parameters, rejected the request with resolved origin: {}", resolvedOrigin);
