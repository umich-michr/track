...
class OriginValidatorImplTest {
  ...
  class ClientIdValidationTests {
    @ParameterizedTest
    @NullAndEmptySource
    void testValidate_clientIdMissing(String clientId) {
    ...
    @Test
    void testValidate_clientConfigurationMissing() {
  ...
  class OriginResolutionTests {
    ...
    void testValidate_resolvesOriginFromRefererWhenOriginHeaderMissing(String referer) {
      when(requestUtil.getParameterValue(StandardParameter.CLIENT_ID, request)).thenReturn(clientId);
    ...
    @Test
    void testValidate_resolvesOriginFromEmailIdForGetRequest() {
      when(requestUtil.getParameterValue(StandardParameter.CLIENT_ID, request)).thenReturn(clientId);
    ...
    @ParameterizedTest
    @NullAndEmptySource
    void testValidate_noOriginFound_GET(String origin) {
      when(requestUtil.getParameterValue(StandardParameter.CLIENT_ID, request)).thenReturn(clientId);
    ...
    @ParameterizedTest
    @NullAndEmptySource
    void testValidate_noOriginFound_POST(String origin) {
      when(requestUtil.getParameterValue(StandardParameter.CLIENT_ID, request)).thenReturn(clientId);
    ...
    @ParameterizedTest
    @ValueSource(strings = {
        "https://localhost:8080/h2-console/login.jsp?jsessionid=12345",
        "https://localhost:8080/",
        "https://localhost:8080/some/path",
        "https://localhost:8080?query=param"
    })
    void testValidate_resolvesBaseUrlFromOrigin(String origin) {
      when(requestUtil.getParameterValue(StandardParameter.CLIENT_ID, request)).thenReturn(clientId);

      when(request.getHeader(HttpHeaders.ORIGIN)).thenReturn(origin);

      ClientConfiguration config = mock(ClientConfiguration.class);
      when(config.getAuthorizedOrigins()).thenReturn(Collections.singletonList("https://localhost:8080"));
      when(clientConfigurationService.getClientConfiguration(clientId)).thenReturn(config);
    ...
    void testValidate_resolvesBaseUrlFromReferer(String referer) {
      when(requestUtil.getParameterValue(StandardParameter.CLIENT_ID, request)).thenReturn(clientId);
  ...
  class AuthorizedOriginValidationTests {
    @Test
    void testValidate_originNotAllowed() {
